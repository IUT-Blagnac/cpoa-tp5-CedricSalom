...
public class CourseController extends JPanel implements Observer, ChangeListener, ActionListener {
	...
	public CourseController(CourseData courses) {
		...
		courses.attach(this);
		Vector<CourseRecord> state = (Vector<CourseRecord>) courses.getUpdate(null);
	...
	public void addCourse(CourseRecord record) {
		...
		this.courseData.notifyObservers(record.getName());
	}
	 ...
	 */
	 public void update(CourseRecord c) {
		//Vector<CourseRecord> courses = c;
		//Vector<CourseRecord> newCourses = courses.getUpdate();
		for (int i = 0; i < sliders.size(); i++) {
			if(c.getName().contentEquals(sliders.get(i).getName()))
				 return;
		}
		this.addCourse((CourseRecord) c);
	} 
	...
	public static void main(String[] args) {
		...
		JScrollPane scrollPane1 = new JScrollPane(pie,
				JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
				JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
		...
		constraints.gridx = 1;
		constraints.gridy = 1;
		frame.getContentPane().add(scrollPane1, constraints);
		frame.pack();
